...
logo.addEventListener('click', () =>{
  let gameName = navMenu.querySelector('li').textContent
...
const getTopGames = (callback) => {
  let endPoint = '/games/top?limit=5'
  setRequest(endPoint,callback)
}

const getStreams = (gameName, callback) => {
  let endPoint = `/streams/?game=${encodeURIComponent(gameName)}&limit=20`
  setRequest(endPoint,callback)
}
...
const renderStreamsCards = (gameName) => {
  ...
  getStreams(gameName, (error, streams) => {
    ...
    appendEmpty()
    appendEmpty()
    //let activedGame = navMenu.querySelector('li').
...
// render navbar menu
getTopGames(function(error, topGames) {
  if(error){
    alert(error)
  }
